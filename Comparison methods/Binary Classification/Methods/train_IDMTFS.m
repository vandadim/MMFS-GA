function [accM,Param,select_ALL,label_P]=train_IDMTFS(X,y,Xtest,ytest)

opt.rho1 = [0.001,0.01,0.1,0.3,0.5,0.7,0.8,0.9,1,5,10,30,40,60,100];
opt.rho2 = [0.001,0.01,0.1,0.3,0.5,0.7,0.8,0.9,1,5,10,30,40,60,100];
...
for j = 1:kfold
    ...
    %===================== Feature selection ==========================
    %%
    s = sum((X_tr{1} - X_tr{2} - X_tr{3} - X_tr{4} - X_tr{5}).^2,2);
    ...
    for k = 1:length(opt.rho1)
        for p = 1:length(opt.rho2)
            ...
            W = IDMTFS(X_tr, y_tr, opts.rho1, opts.rho2, s, opts);
