...
#################################################################################################################
def generateIntBit(intvalue):
    """
    Generate bits for pops based on each data view 
    """
    num=int(np.random.randint(0,intvalue))
    return num
#################################################################################################################
def evalfsolution(X, y):    #Vandad
    """
    ...
    scores3=[]
    for i in range(100):
        train_data_X, test_data_X, train_Y, test_Y = model_selection.train_test_split(X, y, test_size=0.8, random_state=None)
        cl_rf = LinearDiscriminantAnalysis()
        cl_rf.fit(train_data_X, train_Y)
        y_pred=cl_rf.predict(test_data_X)
        scores3.append(balanced_accuracy_score(test_Y, y_pred))
    value = np.mean(scores3)        
    return value
###############################
#################################################################################################################
...
def evalfunctionGA2(individual, allPopulations, data, nf, nb):    #Vandad
    ...
    if X.shape[1]==0:
        ...
        scores=[]
        for i in range(nb):
            train_data_1_X, test_data_1_X, train_Y_1, test_Y_1 = model_selection.train_test_split(X, y, test_size=nf, random_state=None)
            cl_rf = LinearDiscriminantAnalysis()
            cl_rf.fit(train_data_1_X, train_Y_1)
            y_pred=cl_rf.predict(test_data_1_X)
            scores.append(balanced_accuracy_score(test_Y_1, y_pred))
...
################################################################################################################# 
def bvfs(all_Populations, dat, ngen, npop, out_file, Path, procid, pipein, pipeout, sync, ev, queue,seed=None):
    """
    ...
    toolbox.register("genIntBit",generateIntBit,len(all_Populations[0]))
    ...
    toolbox.register("evaluate", evalfunctionGA2, allPopulations = all_Populations, data = dat,  nf = 0.8, nb = 100)        
    ...
    logbook.header = ['gen', 'procid', 'M_F', 'avg', 'max_acc']
    
    # Update the logbook    
    logbook.record(gen=0, procid=procid, M_F=0, avg=0,MAX_ACC=0)
    ...
    for gen in range(1, ngen + 1):        
        ...
        offspring = algorithms.varOr(population, toolbox, lambda_=100, cxpb=0.4, mutpb=0.3)                      
        ...
        fea = [ind.fitness.values[1] for ind in population]
        ...
        logbook.record(gen = gen, procid=procid, M_F=Num_F, avg = macc, MAX_ACC=maxpopp)
    ...
    print("BEST_VIEW_POPULATION =",Best_com)
...
def ivfs(procid, dat, ngen, pipein, pipeout, sync, ev,
          out_file, output_dir, real_A, real_B,view, queue, seed=None):
    """
   ...
   """
    ...
    toolbox.register("evaluate", evalfunction, data = data,  nf = 3, nb = 1)
    ...
    # print("##########################################")  
    print("Best_POP_"+str(procid)+"=",best_pop)    
    queue.put((best_pop))
    ev.set()